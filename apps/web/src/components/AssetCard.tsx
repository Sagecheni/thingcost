...
import { formatMinorCurrency, ticketNumber } from '../lib/format.js';
 ...
 *   票面    名称 + 日均成本               大号等宽
 *   骑缝    撕口                          CardPerforation
 *   存根脚  净成本 | 服役天数           小号等宽
 *
 * 印章刻意不常驻：一屏几十张卡，每张都盖同一枚印，印就从凭信退化成底纹，
 * 而且要跟票面金额抢注意力。方印只在刚写下时落一次（见下方 fresh），
 * 骑缝也不压半印 —— 没有撕走的另一半可对，那半枚证明不了任何事。
 *
...
/* 当票号由 lib/format.ts 提供 —— 详情页要盖同一个号，不能各写一份 */
...
export function AssetCard({ asset, currency = 'CNY' }: AssetCardProps) {
  ...
  const disposed = asset.currentStatus.ownershipState === 'disposed';
  ...
  return (
    ...
    >
      {/* 头联：分类 · 当票号 | 状态。
       * 状态用墨色、分类用淡墨：分类是静态归属，状态是会变的事实，
       * 同级渲染等于把状态藏起来。已处置的退回淡墨——账已了结。 */}
      <div
        className={cn(
      ...
      >
        ...
        <span
          ...
          className={cn(
            'shrink-0',
            disposed ? 'text-muted-foreground' : 'text-foreground',
          )}
        >
       ...
       * 相角贴票：票上贴的物品照，用老相簿的贴法（theme.css 的 photo-mount）。 */}
      {cover ? (
        <div data-slot="photo-mount" className="border-b border-border">
          <img
            src={cover}
            alt=""
            loading="lazy"
      ...
      <div className="flex flex-1 flex-col gap-2 px-3.5 pt-3">
        ...
        {/* 票面金额。mt-auto 把它顶到骑缝线正上方 —— 网格里的卡片被拉成等高，
         * 有封面的和没封面的因此在同一条水平线上出金额。账本的第一条是数字成列，
         * 这一屏的日均成本必须能竖着比。
         * 方印绝对定位在右下角，只在 fresh 的那一下出现 —— 不占流内位置，
         * 落下和消失都不推动金额。印比行高，向上溢出的几像素落在 gap 里。 */}
        <div className="relative mt-auto flex items-end">
          ...
          {fresh ? <SealMark stamped className="absolute right-0 bottom-0" /> : null}
        </div>
      </div>

      <CardPerforation className="mt-3" />

      {/* 当票脚。两栏定宽而不是两端对齐 —— 存根脚也要成列，
       * 一屏卡片的脚部数字才能竖着比。
       * 没有钱可报的时候就报时间 —— 空着一格比补个零诚实。 */}
      <dl className="grid grid-cols-2 gap-x-3 px-3.5 py-2.5 text-[11px] text-muted-foreground">
        {costUnknown ? (
          <div className="flex min-w-0 gap-1">
            <dt className="shrink-0">持有</dt>
